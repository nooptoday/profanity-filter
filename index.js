  ...
  if (process.env.NAPI_RS_NATIVE_LIBRARY_PATH) {
    ...
    if (process.arch === 'arm64') {
      ...
      try {
        ...
        if (
          bindingPackageVersion !== '1.0.5' &&
          process.env.NAPI_RS_ENFORCE_VERSION_CHECK &&
          process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0'
        ) {
          throw new Error(
            `Native binding package version mismatch, expected 1.0.5 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`,
          )
        }
        return binding
      } catch (e) {
...
if (!nativeBinding || process.env.NAPI_RS_FORCE_WASI) {
  ...
  if (!nativeBinding || process.env.NAPI_RS_FORCE_WASI) {
    try {
      wasiBinding = require('@nooptoday/profanity-filter-wasm32-wasi')
      nativeBinding = wasiBinding
    } catch (err) {
      if (process.env.NAPI_RS_FORCE_WASI) {
        if (!wasiBindingError) {
...
module.exports.censor = nativeBinding.censor
